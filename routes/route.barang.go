...
func NewRouteBarang(db *gorm.DB, router *gin.Engine) {
	repository := repositorys.NewRepositoryBarang(db)
	service := services.NewServiceBarang(repository)
	handler := handlers.NewHandlerBarang(service)

	route := router.Group("/api/v1/barang")
